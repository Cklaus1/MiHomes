"""F.3b, executable — reconcile `tasks/build-loop-spec008.md`'s DAG against SPEC-008 §8.
...
**derive the criteria column, never type it** — is what this script enforces here.
    ...
    py scripts/spec008_reconcile.py             # 0 = DAG and spec agree
    py scripts/spec008_reconcile.py --collect   # + every declared node id resolves
...
optional politeness here — the SPEC-008 pre-flight ran it against the three existing-file criteria
...
Labels are parsed as ``A\\d+[a-z]?`` even though SPEC-008's set is plain `A1`–`A25` with no
...
SPEC = ROOT / "docs" / "specs" / "SPEC-008-vendor-discovery-d0.md"
HARNESS = ROOT / "tasks" / "build-loop-spec008.md"
...
# reader meets it.
#
# `tests/unit/test_spec008_reconciler.py::TestCorrectionsExpire` keeps this honest: if a spec
...
PENDING_TESTS_IN_EXISTING_FILES = {
    # **Empty at authoring time, and it will not stay empty.** §8 groups SPEC-008's criteria by
    # FILE, not by group: `test_research.py` holds A4/A5/A6 (G3) *and* A14 (G7);
    # `test_cache.py` holds A7 and A8; `test_web_tools.py` holds A1, A2 and G1.3. So the first
    # group to land creates the file a later group writes into, `--collect` starts checking
    # every node id in it, and the later ones correctly do not resolve yet.
    #
    # This shape recurred three times across SPEC-006 and was rediscovered each time. Add the
    # entry when it happens, annotate why, and **delete it the moment its group lands** —
    # `TestPendingSetExpires` fails if it outlives its group, which is the whole point: an
    # entry that survives exempts a real node id from `--collect` forever.
}

SPEC_NODE_ID_CORRECTIONS = {
    # **Empty, deliberately.** SPEC-006 needed one entry here because §8 named a test that did
    # not exist under that name. SPEC-008's §9 files are all new, so there is nothing to
    # correct *yet* — `--collect` is what would find it, and an entry added before that
    # measurement would be a guess wearing the authority of a correction.
}
...
def main() -> int:
    ...
    # are the same size. SPEC-008 has 19 DAG tasks and 15 criteria — three tasks (G0.1, G0.2,
