"""F.3b, executable — reconcile `tasks/build-loop-spec010.md`'s DAG against SPEC-010 §8.
...
**derive the criteria column, never type it** — is what this script enforces here.
    ...
    py scripts/spec010_reconcile.py             # 0 = DAG and spec agree
    py scripts/spec010_reconcile.py --collect   # + every declared node id resolves
...
optional politeness here — the SPEC-010 pre-flight ran it against the three existing-file criteria
...
Labels are parsed as ``A\\d+[a-z]?`` even though SPEC-010's set is plain `A1`–`A25` with no
...
SPEC = ROOT / "docs" / "specs" / "SPEC-010-email-password-auth.md"
HARNESS = ROOT / "tasks" / "build-loop-spec010.md"
...
# reader meets it.
#
# `tests/unit/test_spec010_reconciler.py::TestCorrectionsExpire` keeps this honest: if a spec
...
PENDING_TESTS_IN_EXISTING_FILES = {
    # **Empty again, and deliberately.** It held one entry between G3 and G6, exactly as §4
    # predicted: `test_password_auth.py` holds A6/A7/A8 (G3) *and* A14 (G6), so landing G3
    # created the file G6 would write into and `--collect` began checking a node id that
    # correctly did not resolve yet.
    #
    # G6 has landed, so the entry is deleted — the discipline that stops a sanctioned gap from
    # becoming a permanent one. `TestPendingSetExpires` fails if an entry outlives its group.
}

SPEC_NODE_ID_CORRECTIONS = {
    # **Empty.** SPEC-010's §9 files are all new and its §8 node ids were written against them
    # in the same commit, so there is nothing to correct.
}
...
def main() -> int:
    ...
    # are the same size. SPEC-010 has 19 DAG tasks and 15 criteria — three tasks (G0.1, G0.2,
