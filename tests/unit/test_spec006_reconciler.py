"""The SPEC-006 reconciler's two exemption tables must not outlive their reasons.

`scripts/spec006_reconcile.py` carries two dicts that let the harness disagree with the spec:
...
SCRIPT = ROOT / "scripts" / "spec006_reconcile.py"


def _load():
    spec = importlib.util.spec_from_file_location("spec006_reconcile", SCRIPT)
...
class TestTheReconcilerItselfHolds:
    ...
    def test_an_ungated_criterion_is_detected(self, rec, tmp_path, monkeypatch):
        ...
        the manual mutation, this rewrites the harness in a temp copy with A11 — the phase's
        definition of done — ungated, and asserts the check fires.
        """
        harness = rec.HARNESS.read_text(encoding="utf-8")
        assert "· A11 ·" in harness, "anchor moved; update this test"
        broken = tmp_path / "harness.md"
        broken.write_text(harness.replace("· A11 ·", "· — ·", 1), encoding="utf-8")

        monkeypatch.setattr(rec, "HARNESS", broken)
        gated = rec.harness_gated(broken.read_text(encoding="utf-8"))
        assert "A11" not in gated, (
            "the mutation did not actually ungate A11 — this test would pass vacuously"
        )
