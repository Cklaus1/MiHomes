"""The SPEC-009 reconciler's two exemption tables must not outlive their reasons.

`scripts/spec009_reconcile.py` carries two dicts that let the harness disagree with the spec:
...
SCRIPT = ROOT / "scripts" / "spec009_reconcile.py"


def _load():
    spec = importlib.util.spec_from_file_location("spec009_reconcile", SCRIPT)
